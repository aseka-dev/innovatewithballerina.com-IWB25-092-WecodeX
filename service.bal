...
service /roomsync on new http:Listener(9090) {

    // Add a new user
    resource function post user(UserInsert data) returns User {
        string hashedInput = hashPassword(data.password);
    ...
    resource function post login(LoginRequest req) returns json|http:Unauthorized {
    string|sql:Error|() result = selectUser(req.email);

    if result is string {
        string hashedInput = hashPassword(req.password);

        io:println("User entered hash: " + hashedInput);
        io:println("DB stored hash: " + result);

        if hashedInput == result {
    ...
    //Get a user  
    resource function get user(string email) returns User|http:NotFound & readonly {
        User|sql:Error|() result = selectUserDetail(email);
...
function hashPassword(string password) returns string {
    byte[] pwBytes = password.toBytes();
    byte[] hash = crypto:hashSha256(pwBytes);
